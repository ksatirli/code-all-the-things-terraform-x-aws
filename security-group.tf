data "aws_vpc" "hack_the_ne" {
  // grab the default (AWS-created) VPC
  default = true
}

resource "aws_security_group" "hack_the_ne" {
  ...
  vpc_id = data.aws_vpc.hack_the_ne.id
...
resource "aws_security_group_rule" "hack_the_ne_allow_all_from_self" {
  ...
  security_group_id = aws_security_group.hack_the_ne.id
}

resource "aws_security_group_rule" "hack_the_ne_allow_http_from_world" {
...
resource "aws_security_group_rule" "hack_the_ne_allow_https_from_world" {
